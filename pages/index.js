    ...
    <Layout>
      <LazyMotion features={domAnimation}>
        ...
        >
          ...
          <button onClick={next} className="absolute bottom-[80px] md:bottom-[81.75vh] right-0 z-10 w-1/2 h-12 flex items-start justify-end outline-none border-none group" onMouseEnter={() => setHoveringNext(true)} onMouseLeave={() => setHoveringNext(false)}>
          ...
          <button onClick={prev} className="absolute bottom-[80px] md:bottom-[81.75vh] left-0 z-10 w-1/2 h-12 flex items-start justify-start outline-none border-none group" onMouseEnter={() => setHoveringPrev(true)} onMouseLeave={() => setHoveringPrev(false)}>
            <span className={`text-off-black block px-4 md:px-5 py-2 font-mono text-sm uppercase transition-all ease-in-out duration-[400ms]  group-hover:opacity-100`}>
            <span className="block relative overflow-hidden">
